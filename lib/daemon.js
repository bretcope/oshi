...
//	};
	...
	/**
	 * @param options {Array|string|ChildConfig}
	...
	this.info = function (options, callback)
	{
		...
		var children = options.map(ChildConfig);		
		children = children.filter(function (child) 
		{
			return child.groupName in _groups;
		});

		var result = {};
		Async.each(children, function (child, done)
		{
			var group = _groups[child.groupName];
			group.info(child, function (err, info)
			{
				if (err) return done(err);

				var groupInfo = result[child.groupName] || (result[child.groupName] = {});
				groupInfo[child.port] = info;
				done();
			});
		}, 
		function (err)
	...
	function onGroupBroadcast (e, info, data)
	{
		...
		var message = { 
			...
			source: {
				...
				group: e.origin.groupName,
				port: e.origin.port
			}
		};

		for (var j in _groups)
			_groups[j].broadcast(message, broadcastToSelf);
