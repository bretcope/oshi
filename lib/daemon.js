...
//	};
	...
	this.prepare = function (groupConfig, callback)
	{
		...
		try
		{
			...
			else
			{
				...
				for (var eventName in groupEventHandlers)
					_groups[groupConfig.name].attach(eventName, groupEventHandlers[eventName]);
	...
	{
		if (!groupEventHandlers.hasOwnProperty(e.event)) 
	...
	function onGroupBroadcast (e, info, data)
	{
		...
		for (var j in _groups)
			_groups[j].broadcast(data.event, broadcastData, source, broadcastToSelf);
