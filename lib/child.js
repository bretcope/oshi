	...
	Object.defineProperty(this, 'process',
	{
		...
		set: function (proc)
		{
			if (_process)
			{
				...
				try
				{
					debug(_this.shortName + ' attempting to unpipe stdout and stderr');
					_process.stdout.unpipe();
					_process.stderr.unpipe();
					debug(_this.shortName + ' stdout and stderr unpiped');
				}
				catch (ex)
				{
				...
				try
				{
					debug(_this.shortName + ' attempting to close log streams');
					_logStreams.out.end();
					_logStreams.err.end();

					_logStreams = {};
					debug(_this.shortName + ' log streams closed');
				}
				catch (ex)
				{
					// don't really care if unpipe throws an exception... though I don't think it ever should
			...
			if (_process)
			{
				...
				if (logDir)
				{
					...
					for (var name in outputs)
					{
						...
						debug(_this.shortName + ' piping std' + name);
