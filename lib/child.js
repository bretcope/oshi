...
Child.prototype.start = function (newConfig, callback, stopResponse)
{
	...
	{
		mkdirp(daemonConf.logDir);
		var filePath = Path.resolve(daemonConf.logDir, _this.groupName + '_' + _this.port + '.' + type + '.log');
		return fs.createWriteStream(filePath, { flags: 'a' });
	}

	this.process.stdout.pipe(logFile('out'));
	this.process.stderr.pipe(logFile('err'));
