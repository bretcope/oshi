...
function Child (group, config)
{
	...
	function disconnectProcess (proc)
	{
		...
		proc.removeListener('error', _this.onError);
		proc.removeListener('exit', _this.onExit);
		proc.removeListener('message', _this.onMessage);
		...
		try
		{
			_this.logStreams.out.end();
			_this.logStreams.err.end();
			_this.logStreams = {};
		}
		catch (ex)
		{
	...
	function connectProcess (proc)
	{
		...
		proc.on('error', _this.onError);
		proc.on('exit', _this.onExit);
		proc.on('message', _this.onMessage);

		// setup log files
		// TODO: Make these rotatable
		var logDir = _this.group.daemon.config.logDir;
		createLogfile.call(_this, proc.stdout, 'out', logDir);
		createLogfile.call(_this, proc.stderr, 'err', logDir);
	}
	...
	Object.defineProperty(this, 'process', {
		...
		set: function (proc)
		{
			disconnectProcess(_process);
			_process = proc;
			connectProcess(_process);
		}
	});
...
function getLogfilePath (logDir, name)
{
	return Path.resolve(logDir, this.groupName + '_' + this.port + '.' + name + '.log');
}
